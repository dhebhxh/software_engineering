...
class CollideSystem {
    constructor(gameEntities) {
        this.gameEntities = gameEntities;
    }
    
    updateGameEntities(gameEntities) {
        this.gameEntites = gameEntities;
    }
    
    collideDetection() {

        for(let i = 0; i < this.gameEntities.length; i++) {
            for(let j = i + 1; j < this.gameEntites.length; j++) {
            //两个静态的直接跳过检测，或者一个触发一个静态也跳过
                if(this.collideTest(this.gameEntites[i], this.gameEntites[j])) {
                    let collideResult = this.correctPosition(this.gameEntites[i], this.gameEntites[j]);
                    this.gameEntites[i].collideResponse(collideResult);//需要包含发生这次碰撞的另外一个对象是谁
                    this.gameEntites[j].collideResponse(collideResult);
                }
            }
        }
    }
        ...
        // if(this.collideTestRectRect(this.character, this.cliff)) {
        //     this.character.reset(10, 80);
        // }

        // for(const entity of this.entities) {
            
        //     if(this.collideTestRectRect(this.character, entity)) {
        //         const collideSide = this.correctPosition(this.character, entity);
        ...
        //     for(const entity of this.entities) {
        //         if(this.collideTestRectRect(this.replayer, entity)) {
        //             const collideSide = this.correctPosition(this.replayer, entity);
        ...
        //     if(this.collideTestRectRect(this.character, this.replayer)) {
        //         const collideSide = this.correctPosition(this.character, this.replayer);
        //         this.character.collideResponse(collideSide);
        //         if(collideSide === "bottom") {
        //             charIsGround = true;
    ...
    correctPosition(r1, r2) {
        //有的需要修改位置，比如静态和动态，两个动态
        //有的不需要修改位置，比如触发型的，机关，按钮，终点线，陷阱
        let collisionSide = "";
...
class Entity {
    constructor(x, y, w, h, bodyType, colliderType) {
        this.x = x;
        this.y = y;
        this.w = w;
        this.h = h;
        this.bodyType = bodyType;//text
        this.colliderType = colliderType;//text
...
class Character extends GameEntity {
    constructor(x, y, w, h) {
        ...
        this.accY = this.gravity;//重力加速度一直存在
    }
...
class Scene {
    constructor(character, home, ground1, ground2, platform, cliff, wall1, wall2) {
        ...
        this.collide = new CollideSystem(this.gameEntites);
    ...
    addGameEntity(newGameEntity) {
        this.entities.push(newGameEntity);
        collide.updateGameEntities();
    }
